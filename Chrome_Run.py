...
def Chrome_Run_Main():
    test_files = [
        ...
        # -> 20251212 회원가입 완료 전 플로우 테스트 동작 코드 수정 (개발 로직 변경으로 인해 20260224 돌림 -> 20260323 도릴 예정)
        ...
        # 84번 - 에디터픽 상세 > 링크드인 연동 확인 (20260206 - 링크드인 봇체크 및 Jenkins 체크 시 막힘으로 인한 오류 지속 발생으로 disable 처리)
        ###"tests/editorspick_detail_linkedin_link_move_check.py",
        # 85번 - 세일즈 에이전트 UI 확인
        #"tests/salesagent_ui_check.py",
        # 86번 - 세일즈 에이전트 > Starter > 구독 결제 완료 전까지 동작 확인 (테스트 케이스 동작 설명 수정)
        #"tests/salesagent_starter_toss_payments_1_2page_input_check.py",
        # 87번 - 20251125 세일즈 에이전트 > 상단 > 상담요청하기 등록 확인 -
        ...
        ######### ----- 테스트 스크립트 이슈 또는 테스트 스크립트 제외 항목들 ----- #########
        ...
        # 90번 - 세일즈 에이전트 > 플랜 혜택 보기 > 상담 요청하기 문의사항 등록
        # "tests/salesagent_plan_benefits_request_a_consultation_register.py"
