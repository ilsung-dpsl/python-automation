...
def test_dashboard_mylist_contacts_count_check(page):
  ...
  #  page.get_by_role("button", name="Start Now").click()
    ...
    #20251205 - 마이리스트 저장 카운트 개수 수정
    assert "6,621" in page.content(), "대시보드 > 마이리스트 영역 > 저장 카운트 개수 확인 실패 3"
    assert "확인되지 않은" in page.content(), "대시보드 > 마이리스트 영역 > 미확인된 문구 노출 확인 실패 4"
    #20251205 - 마이리스트 확인되지 않은 카운트 개수 수정
    assert "6,393" in page.content(), "대시보드 > 마이리스트 영역 > 미확인된 연락처 카운트 개수 확인 실패 5"
