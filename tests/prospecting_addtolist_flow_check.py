...
def test_prospecting_addtolist_flow_check(page):
    ...
    page.get_by_placeholder("예: 일본 화장품 제조사 세일즈 매니저").fill("물류 회사를 찾아줘")
    ...
    assert "Antoinette Payne" in page.content(), "Default 폴더 test 1의 리드 1번 성함 확인 실패 - 리스트에 추가 실패 1"
    assert "Kesha Rockette" in page.content(), "Default 폴더 test 1의 리드 2번 성함 확인 실패 - 리스트에 추가 실패 1"
    ...
    assert "Antoinette Payne" in page.content(), "일반 폴더 test 1의 리드 1번 성함 확인 실패 - 리스트에 추가 실패 2"
    assert "Kesha Rockette" in page.content(), "일반 폴더 test 1의 리드 2번 성함 확인 실패 - 리스트에 추가 실패 2"
    ...
    page.locator(".ml-\\[2px\\] > .w-fit > .p-\\[2px\\]").first.click()
    page.locator("div:nth-child(2) > div > .ml-\\[2px\\] > .w-fit > .p-\\[2px\\]").click()
    page.wait_for_timeout(1000)
