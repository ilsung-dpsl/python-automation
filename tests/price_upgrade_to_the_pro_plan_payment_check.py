...
def test_price_upgrade_to_the_pro_plan_payment_check(page):
    ...
    page.keyboard.type(config.CARD2_CARD_NO, delay=100)
    page.wait_for_timeout(500)
    page.get_by_placeholder("MM/YY").fill(config.CARD2_VALID_THRU)
    page.wait_for_timeout(500)
    page.get_by_placeholder("CVC를 입력하세요").fill(config.CARD2_CVC_NO)
    page.wait_for_timeout(500)
    page.get_by_placeholder("카드 소지자명을 입력하세요").fill(config.CARD2_CONSUMER_NAME)
    page.wait_for_timeout(500)
