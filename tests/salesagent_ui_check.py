...
def test_salesagent_ui_check(page):
    ...
    assert "정부 지원 바우처 혜택" == page.get_by_text("정부 지원 바우처 혜택").inner_text(), \
        "세일즈 에이전트 UI > 정부 지원 바우처 혜택 타이틀 문구 노출 확인 실패 - 세일즈 에이전트 UI  확인 실패 4"
    # 20251202 - 세일즈 에이전트 랜딩 페이지 > 정부 지원 바우처 혜택 > 안내 문구 노출 확인 코드 추가
    assert "딥세일즈의 에이전트 서비스는 다양한 정부 부처 및 공공기관이 주관하는 사업과 함께 합니다." == page.get_by_text("딥세일즈의 에이전트 서비스는 다양한 정부 부처 및 공공기관이 주관하는 사업과 함께 합니다").inner_text(), \
    ...
    assert "시작하기" == page.get_by_role("button", name="시작하기").nth(3).inner_text(), \
