...
def test_salesagent_scale_toss_payments_1_2page_input_check(mobile_page):
    ...
    print("MO Web - 로그인 완료 후 세일즈 에이전트 랜딩페이지 진입")

    mobile_page.get_by_role("button", name="시작하기").nth(1).tap()
    mobile_page.wait_for_timeout(1000)

    print("MO Web - 토스페이먼츠 결제창 출력 상태 완료")
    ...
    for i in range(1, 5, 1):
        ...
        mobile_page.wait_for_timeout(500)
        print(f"MO Web - 토스페이먼츠 결제창 1 > 카드번호 마지막 뒷자리 입력 카운트 : {i}")
    ...
    assert "4033" == mobile_page.get_by_role("textbox", name="카드번호 1 ~ 4 자리").input_value(), \
        "MO Web - 토스페이먼츠 결제창 1 > 카드번호 1~4자리 번호 확인 실패 - 세일즈 에이전트 > Scale > 구독 결제 완료 전까지 동작 확인 1"
    assert "0201" == mobile_page.get_by_role("textbox", name="카드번호 5 ~ 8 자리").input_value(), \
        "MO Web - 토스페이먼츠 결제창 1 > 카드번호 5~8자리 번호 확인 실패 - 세일즈 에이전트 > Scale > 구독 결제 완료 전까지 동작 확인 2"
    assert "6000" == mobile_page.get_by_role("textbox", name="카드번호 9 ~ 12 자리").input_value(), \
        "MO Web - 토스페이먼츠 결제창 1 > 카드번호 9~12자리 번호 확인 실패 - 세일즈 에이전트 > Scale > 구독 결제 완료 전까지 동작 확인 3"
    ...
    assert "12/26" == mobile_page.get_by_role("textbox", name="카드 유효기간").input_value(), \
        "MO Web - 토스페이먼츠 결제창 1 > 카드 유효기간 확인 실패 - 세일즈 에이전트 > Scale > 구독 결제 완료 전까지 동작 확인 4"
    ...
    print("MO Web - 토스페이먼츠 결제창2 본인 인증 창 노출 확인")
    ...
    assert "000000" == mobile_page.get_by_role("textbox", name="인증번호").input_value(), \
        "MO Web - 토스페이먼츠 결제창 2-2 (본인인증) > 인증번호 인증창 > 인증번호 입력값 확인 실패 - 세일즈 에이전트 > Scale > 구독 결제 완료 전까지 동작 확인 5"

    mobile_page.get_by_role("button", name="뒤로가기").tap()
    mobile_page.wait_for_timeout(2000)

    assert "백일성" == mobile_page.get_by_role("textbox", name="이름").input_value(), \
        "MO Web - 토스페이먼츠 결제창 2-1 (본인인증) > 이름 확인 실패 - 세일즈 에이전트 > Scale > 구독 결제 완료 전까지 동작 확인 6"
    assert "870724" == mobile_page.get_by_role("textbox", name="주민등록번호 생년월일").input_value(), \
        "MO Web - 토스페이먼츠 결제창 2-1 (본인인증) > 주민등록번호 생년월일 확인 실패 - 세일즈 에이전트 > Scale > 구독 결제 완료 전까지 동작 확인 7"
    assert "1" == mobile_page.get_by_role("textbox", name="주민등록번호 성별").input_value(), \
        "MO Web - 토스페이먼츠 결제창 2-1 (본인인증) > 주민등록번호 성별 확인 실패 - 세일즈 에이전트 > Scale > 구독 결제 완료 전까지 동작 확인 8"
    assert "01041342385" == mobile_page.get_by_placeholder("휴대폰번호").input_value(), \
        "MO Web - 토스페이먼츠 결제창 2-1 (본인인증) > 휴대폰번호 확인 실패 - 세일즈 에이전트 > Scale > 구독 결제 완료 전까지 동작 확인 9"
