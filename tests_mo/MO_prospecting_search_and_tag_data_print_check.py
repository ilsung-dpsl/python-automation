...
def test_MO_prospecting_search_and_tag_data_print_check(mobile_page):
    ...
    assert "도매 화장품" in mobile_page.content(), \
        "MO Web - 탐색하기 > AI 산업군 > 키워드 1 노출 실패"
    #    assert "도매 의약품 및 잡화" in page.content(), "탐색하기 > AI 산업군 > 키워드 2 노출 실패"
    assert "소매 화장품" in mobile_page.content(), \
        "MO Web - 탐색하기 > AI 산업군 > 키워드 3 노출 실패"

    assert "대한민국" in mobile_page.content(), \
        "MO Web - 탐색하기 > 위치 필터 > 위치 키워드 1 노출 실패"

    assert "Wholesale" in mobile_page.content(), \
        "MO Web - 탐색하기 > 리드 데이터 > 산업군 1 - Wholesale 확인 실패"
    assert "Retail" in mobile_page.content(), \
        "MO Web - 탐색하기 > 리드 데이터 > 산업군 2 - Retail 확인 실패"

    assert "창준 백" in mobile_page.content(), \
        "MO Web - 탐색하기 > 리드 데이터 1 성함 확인 실패"
    assert "준용 안" in mobile_page.content(), \
        "MO Web - 탐색하기 > 리드 데이터 2 성함 확인 실패"
