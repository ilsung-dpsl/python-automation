...
def test_MO_dashboard_gotomyaccount_move(mobile_page):
    ...
    # 20260129 - 대시보드 > 내 계정으로 이동하기 > 선택 시 앨리먼트 나올 때까지 타임아웃 대기 수정
    mobile_page.get_by_text("내 계정으로 이동하기 >").tap(timeout=10000)
    mobile_page.wait_for_timeout(2000)

    print("MO Web - 계정 및 설정 > 내 프로필 페이지로 이동")

    assert "계정 및 설정" in mobile_page.content(), \
        "MO Web - 계정 및 설정 페이지 이동 실패 1"
    assert "내 프로필" in mobile_page.content(), \
        "MO Web - 계정 및 설정 > 내 프로필 이동 실패 2"
    assert "업로드" in mobile_page.content(), \
        "MO Web - 계정 및 설정 > 내 프로필 > 프로필 영역 노출 실패 3"
    assert "최초 등록일 : 2025년 03월 24일" in mobile_page.content(), \
        "MO Web - 계정 및 설정 > 내 프로필 > 최초 등록일 노출 실패 4"
