...
def test_MO_usageanalysis_section_check(mobile_page):
    ...
    assert "산업" in mobile_page.content(), "Usage Analysis > 산업 체크 실패"
    assert "부서" in mobile_page.content(), "Usage Analysis > 부서 체크 실패"
    assert "직위" in mobile_page.content(), "Usage Analysis > 직위 체크 실패"
    assert "Wholesale Cosmetics" in mobile_page.content(), "Usage Analysis > 산업 > 임의 항목 체크 실패"
    assert "General Management" in mobile_page.content(), "Usage Analysis > 부서 > 임의 항목 체크 실패"
    assert "Junior" in mobile_page.content(), "Usage Analysis > 직위 > 임의 항목 체크 실패"
