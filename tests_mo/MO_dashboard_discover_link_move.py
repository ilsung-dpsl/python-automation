...
def test_MO_dashboard_discover_link_move(mobile_page):
    ...
    assert "잠재고객 '" in mobile_page.content(), \
        "MO Web > 대시보드 > 발견하기 > 1번 항목 > 탐색하기 연동 후 잠재고객 노출 확인 실패 1"
    assert "Religious Institutions" in mobile_page.content(), \
        "MO Web > 대시보드 > 발견하기 > 1번 항목 > 탐색하기 연동 후 검색결과의 카드 타이틀 문구 확인 실패 2"
    assert "Lenneke Keuning" in mobile_page.content(), \
        "MO Web > 탐색하기 연동 후 리드 데이터 출력 실패 3"

    print("--- MO Web - 발행일 기준 최신 상위 6개 Discover 노출, 클릭 시 Discover 항목 선택 결과로 연결 1 -> 성공")

    mobile_page.get_by_role("link").filter(has_text="대시보드").tap(timeout=10000)
    mobile_page.wait_for_timeout(2000)

    print("MO Web - 대시보드 재진입 완료")

    mobile_page.get_by_text("미국 도매 알코올 음료 업계, 직원 수 50-200").tap()
    mobile_page.wait_for_timeout(3000)

    assert "Wholesale Alcoholic" in mobile_page.content(), \
        "MO Web - 대시보드 > 발견하기 > 5번 항목 > 탐색하기 연동 실패 2"
    assert "Jamie Pollack" in mobile_page.content(), \
        "MO Web - 탐색하기 연동 후 리드 데이터 출력 실패 2"

    print("--- MO Web - 발행일 기준 최신 상위 6개 Discover 노출, 클릭 시 Discover 항목 선택 결과로 연결 2 -> 성공")
