...
def test_dashboard_mylist_contacts_count_check(mobile_page):
    ...
    assert "마이 리스트" in mobile_page.content(), "대시보드 > 마이리스트 영역 > 타이틀 문구 노출 확인 실패 1"
    assert "저장" in mobile_page.content(), "대시보드 > 마이리스트 영역 > 저장 문구 노출 확인 실패 2"
    # 20251211 - 마이리스트 저장 카운트 개수 재수정
    assert "6,602" in mobile_page.content(), "대시보드 > 마이리스트 영역 > 저장 카운트 개수 확인 실패 3"
    assert "확인되지 않은" in mobile_page.content(), "대시보드 > 마이리스트 영역 > 미확인된 문구 노출 확인 실패 4"
    # 20251230 - 마이리스트 확인되지 않은 카운트 개수 재수정
    assert "6,385" in mobile_page.content(), "대시보드 > 마이리스트 영역 > 미확인된 연락처 카운트 개수 확인 실패 5"
